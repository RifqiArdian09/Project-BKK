...
<body>
    ...
    <main class="container mx-auto px-5 py-8">
        <div class="text-center mb-10">
            <h2 class="text-3xl md:text-4xl font-bold text-gray-800 mb-2">INFO ALUMNI</h2>
            <div class="w-20 h-1 bg-teal-600 mx-auto"></div>
            <p class="mt-4 text-gray-600 max-w-2xl mx-auto">Temukan informasi untuk lulusan SMKN 1 Bengkulu</p>
        </div>
                        ...
                        @else
                            <div class="w-full h-full flex items-center justify-center text-gray-400">
                                <div class="text-center">
                                    ...
                                    <p class="text-sm">Alumni Kerja</p>
                                </div>
                            </div>
                        @endif
                        
                        @if ($alumni->tanggal_berakhir && \Carbon\Carbon::parse($alumni->tanggal_berakhir)->isPast())
                            <span class="status-badge bg-red-500">Kadaluarsa</span>
                        @elseif ($alumni->tanggal_berakhir && \Carbon\Carbon::parse($alumni->tanggal_berakhir)->diffInDays() < 7)
                    ...
                    <div class="flex-grow">
                        ...
                        <div class="flex flex-wrap mb-3">
                            <span class="info-badge">
                                <i class="fas fa-calendar-day"></i> {{ \Carbon\Carbon::parse($alumni->tanggal_diposting)->format('d M Y') }}
                            </span>
                            @if ($alumni->tanggal_berakhir)
                                <span class="info-badge {{ \Carbon\Carbon::parse($alumni->tanggal_berakhir)->diffInDays() < 3 ? 'urgent-badge' : '' }}">
                                    <i class="fas fa-calendar-times"></i> {{ \Carbon\Carbon::parse($alumni->tanggal_berakhir)->format('d M Y') }}
                                </span>
                            @endif
                        </div>
                ...
                <div id="modal-{{ $alumni->id }}" class="modal">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="mb-8">
                                @if ($alumni->gambar)
                                    <div class="image-container w-full mb-6 rounded-xl overflow-hidden shadow-lg">
                                        <img 
                                            src="{{ asset('storage/' . $alumni->gambar) }}" 
                                            alt="Gambar {{ $alumni->judul }}" 
                                            class="w-full max-h-[400px] object-contain bg-gray-100 zoomable-image"
                                ...
                                <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-4">
                                    <div>
                                        <h2 class="text-3xl font-bold text-gray-800">{{ $alumni->judul }}</h2>
                                        @if ($alumni->subjudul)
                                ...
                                <div class="author-info mt-3">
                                    <i class="fas fa-user"></i>
                                    <span>Diposting oleh: {{ $alumni->author }}</span>
                                </div>
        ...
        @if ($infoAlumni->hasPages())
        <div class="pagination-container">
            <div class="pagination">
                {{-- Previous Page Link --}}
                @if ($infoAlumni->onFirstPage())
                    <span class="pagination-item pagination-disabled pagination-arrow">
                        <i class="fas fa-chevron-left"></i>
                    </span>
                @else
                    <a href="{{ $infoAlumni->previousPageUrl() }}" class="pagination-item pagination-link pagination-arrow">
                        <i class="fas fa-chevron-left"></i>
                    </a>
                @endif

                {{-- Pagination Elements --}}
                @foreach ($infoAlumni->getUrlRange(1, $infoAlumni->lastPage()) as $page => $url)
                    @if ($page == $infoAlumni->currentPage())
                ...
                {{-- Next Page Link --}}
                @if ($infoAlumni->hasMorePages())
                    <a href="{{ $infoAlumni->nextPageUrl() }}" class="pagination-item pagination-link pagination-arrow">
            ...
            <p class="pagination-info">
                Menampilkan {{ $infoAlumni->firstItem() }} - {{ $infoAlumni->lastItem() }} dari {{ $infoAlumni->total() }} Alumni
            </p>
        </div>
        @endif
    </main>
